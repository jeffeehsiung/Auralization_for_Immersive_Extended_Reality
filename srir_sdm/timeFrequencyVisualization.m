    ...
    for k = 1:length(t)
        switch lower(p.DOI)
            case {'backward'} % USEFUL FOR SMALL ROOMS
                ...
                H = abs(fft(P{s}(t2:t_end),nfft)/sqrt(nfft)).^2;
                disp('BACKWARD INTEGRATION');
                
            case {'forward'} % USEFUL FOR LARGE ROOMS
                % ----- FORWARD INTEGRATION from direct sound to t ------
                t2 = min(t_ds+t(k),t_end);
                H = abs(fft(P{s}(t_ds:t2),nfft)/sqrt(nfft)).^2;
                disp('FORWARD INTEGRATION');
    ...
    case {'backward'}
        for k = 1:length(t)
            ...
            plotFreqResponse(HS(:,k),f, p.colors(k,:), ...
                ...
                p.dBSpacing);
            disp('find the maximum for plotting: backward');
        end
    case {'forward'}
        HS = HS(:,end:-1:1);
        for k = 1:length(t)
            ...
            plotFreqResponse(HS(:,k), f, p.colors(length(t)-k+1,:), ...
                ...
                p.dBDynamics, maxdB, p.name, p.plotStyle, flagGrid, ...
                p.dBSpacing);
            disp('find the maximum for plotting: forward');
        end
end
